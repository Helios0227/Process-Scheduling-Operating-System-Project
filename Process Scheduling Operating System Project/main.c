...
//  operating
//
//  Created by Hüseyin Koç on 8.12.2023.
//
...
void sjf_scheduler()
{
    ...
    while (job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter)
        {
            p->flag = PROCESS_FLAG_READY;
            dequeue(job_queue, p->pid);
            enqueue_orderby_burst_time(ready_queue, p); // Sıralama işlemi burst time'a göre yapılır
        }
        else
        {
            break;
    ...
    // Check if any process has finished IO
    current = waiting_queue->head;
    while (current != NULL)
    {
        p = current->data;
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            dequeue(waiting_queue, p->pid);
            if (p->program_counter >= p->burst_time)
            {
                exit_process(p);
            }
            else
    ...
    {
        printf("P%d executing (T=%d)\n", p->pid, cpu_counter - 1);
        p->flag = PROCESS_FLAG_RUNNING;
        dequeue(ready_queue, p->pid);
        
        p->cpu_time = cpu_counter;
        p->program_counter++;

        if (p->program_counter >= p->burst_time_before_io)
        {
            enter_io(p);
        }
